...
def plot_mask_2d(img, mask_2d, test_id, bbox_list=None, class_probs=None, ax=None, title=None):
    if ax is None:
    ...
    fig.savefig(stage1_test_dir + '/' + test_id + '/AGAIN_Unet_cluster_prediction.png')

def get_test_mask(img_id):
    img_path = stage1_test_dir + '/' + img_id
    mask_files = next(os.walk(img_path + '/AGAIN_Unet_cluster/'))[2]
    mask_files = sorted(mask_files)
    num_masks = len(mask_files)
    mask = imread(img_path + '/AGAIN_Unet_cluster/' + mask_files[0])
    mask_2d = np.zeros(shape=[mask.shape[0], mask.shape[1]], dtype=np.int)

    for i_mask, mask_file in enumerate(mask_files):

        current_mask = imread(img_path + '/AGAIN_Unet_cluster/' + mask_file)
...
def sanity_check(max_n_images = 5):
    ...
    for i, test_id in enumerate(test_id_list):
        ...
        plot_mask_2d(img=img, mask_2d=mask_2d,test_id=test_id)
        # plt.show()
...
def decode_submission(filename, visualize=False):
    ...
    for i, test_id in enumerate(test_id_list):
        ...
        mask_dir = stage1_test_dir + '/' + test_id + '/AGAIN_Unet_cluster/'
...
if __name__ == '__main__':
    filename = '../AGAIN_Unet_cluster.csv'
