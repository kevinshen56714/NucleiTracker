...
df = pd.read_csv('C:\\Users\\liukevin\\Desktop\\project_DM\\clustered_stage1_train.csv',sep=',')
df1 = pd.read_csv('C:\\Users\\liukevin\\Desktop\\project_DM\\clustered_stage1_test.csv',sep=',')
cluster=[[],[],[],[]]
for i in range(len(df)):
	if df['cluster-id'][i]==0:
		cluster[0].append(df['ImageId'][i])
	elif df['cluster-id'][i]==1:
		cluster[1].append(df['ImageId'][i])
	elif df['cluster-id'][i]==2:
		cluster[2].append(df['ImageId'][i])
	elif df['cluster-id'][i]==3:
		cluster[3].append(df['ImageId'][i])

		
		
cluster_test=[[],[],[],[]]
for i in range(len(df1)):
	if df1['cluster-id'][i]==0:
		cluster_test[0].append(df1['ImageId'][i])
	elif df1['cluster-id'][i]==1:
		cluster_test[1].append(df1['ImageId'][i])
	elif df1['cluster-id'][i]==2:
		cluster_test[2].append(df1['ImageId'][i])

IdInCluster=1
# Get train and test IDs
train_ids = cluster[IdInCluster]
test_ids = cluster_test[IdInCluster]
...
def UNET(a,b,c,d):
	...
	results = model.fit(a,b,batch_size=16,verbose=1,epochs=30,validation_data=(c,d),callbacks = [earlystopper, checkpointer, MetricsCheckpoint('logs')])
